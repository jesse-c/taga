...
Router.route('/room/:_id', {
  ...
  waitOn: function() {
    ...
    return [ 
             ...
             Meteor.subscribe('dataSources'),
             Meteor.subscribe('dataSourceInstances', this.params._id),
             Meteor.subscribe('#r'),
             Meteor.subscribe('#g'),
             Meteor.subscribe('#b'),
             Meteor.subscribe('sensor'),
             Meteor.subscribe('submitted'),
...
Router.route('/room/:_id/object/:_objectId', {
  name: 'object',
  onBeforeAction: function() {
    // Check for the existence of a room
    var room = Rooms.findOne(this.params._id);
  ...
  data: function() {
    return ObjectInstances.findOne(this.params._objectId);
  },
  action: function() {
    var obj = ObjectInstances.findOne(this.params._objectId);
    this.render(obj.id + '_mobile');
  }
});
